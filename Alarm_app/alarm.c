...
int main(int argc, char* argv[])
{
  if(argc!=3)
  {
    printf("Usage %s <hour-24> <min>\n", argv[0]);
    return 1;
  }
  pthread_t tid1, tid2, tid3;
  
  int hour_user=atoi(argv[1]);
  int min_user=atoi(argv[2]);
  ...
  int time_diff_in_sec=hour_diff*60*60 + min_diff*60;

  printf("Alarm will ruin your sleep in %d hour(s) and %d minutes\ntim_diff_in_sec=%d\n", hour_diff, min_diff, time_diff_in_sec);
